...
public class SignatureUtil {
     ...
     * 
     * This should be the favored mechanism, as the XML Digital Signature APIs operate on DOM elements, thusly it'd be more efficient than the JDOM
     * variant, which uses this anyway and eliminates the need for converting between types
     * */
    public static void signElement(Element domElement, KeyPair keyPair) throws XMLSignatureException {
        ...
        SignedInfo si;
        try {
            si = fac.newSignedInfo(fac.newCanonicalizationMethod(DOMCanonicalizationMethod.EXCLUSIVE_WITH_COMMENTS, (XMLStructure) null),
                    fac.newSignatureMethod(ECDSA_SHA1, null), Collections.singletonList(ref));
        } catch (NoSuchAlgorithmException | InvalidAlgorithmParameterException e) {
    ...
    /**
     * Gets the validation context from an element, necessary for extracting the signature
     */
