...
public class TestUtilTest {
    ...
    public void getElementsByTagName() throws Exception {
        ...
        Node test1 = entryElement.getOwnerDocument().createElementNS(AtomSigner.XMLNS, "test");
        Text firstGuy = entryElement.getOwnerDocument().createTextNode("first guy");
        Node test2 = entryElement.getOwnerDocument().createElementNS(AtomSigner.XMLNS, "test");
        ...
        List<Node> titleNodes = TestUtil.getElementsByTagName(entryElement, AtomSigner.XMLNS, "title");
        assertEquals(1, titleNodes.size());

        List<Node> testNodes = TestUtil.getElementsByTagName(entryElement, AtomSigner.XMLNS, "test");
