...
public class RestEndpointControllerIntegrationTest {
	...
	public void testReadFeed() throws Exception {
		...
		rest.postForEntity("http://localhost:8080/publish", signer.newEntry("Another new post!", keyPair), String.class);
		rest.postForEntity("http://localhost:8080/publish", signer.newEntry("Second entry title!", keyPair),
		        String.class);
		rest.postForEntity("http://localhost:8080/publish", signer.newEntry("Third time's the charm!", keyPair),
		        String.class);
		...
		assertTrue(responseBody.contains("Another new post!"));
		...
		int expectedLast = responseBody.indexOf("Another new post!");
